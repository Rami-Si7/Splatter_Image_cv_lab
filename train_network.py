...
def main(cfg: DictConfig):
    ...
    for num_epoch in range((cfg.opt.iterations + 1 - first_iter)// len(dataloader) + 1):
        ...
        for data, target_batch in zip(dataloader, target_dataloader):
            ...
            if iteration > 3000:
                custom_loss = custom_loss_fn_batched(target_reconstructions, gaussian_splats) * custom_lambda
            ...
            with torch.no_grad():
                if iteration % cfg.logging.loss_log == 0 and fabric.is_global_zero:
                    wandb.log({"total_loss": np.log10(total_loss.item() + 1e-8), "custom_lambda":custom_lambda, "base_loss": np.log10(base_loss.item() + 1e-8)}, step=iteration)
                    if iteration > 3000:
                      wandb.log({"custom_loss": custom_loss}, step=iteration)
                    if cfg.opt.lambda_lpips != 0:
            ...
            for fname_to_save in fnames_to_save:
                ...
                if (iteration + 1) % 1000 == 0 or fname_to_save == "model_best.pth":
                    ...
                    drive_save_dir = "/content/drive/MyDrive/final_modified"
